...
class Energa24CostTrackingSensor(SensorEntity):
    ...
    def state(self):
        if self._state is None:
            return None
        # Check for None just in case, though latest_price filters for not None
        if self._state.gross_amount is None or self._state.wear is None or self._state.wear == 0:
            return None
